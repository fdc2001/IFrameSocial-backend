...
class PublicationController extends Controller
{
    public function feed(Request $request) {
        ...
        $data=Publication::whereIn('user', $ids)->paginate();
        return response()->json($data);
    }

    public function myPublications(Request $request) {
        $user= $request->get('user');
        $data= Publication::where('user', '=', $user->id)->where('status', '=', 1)->paginate();
        return response()->json($data);
    }
    ...
    public function getTumbnail(Request $request, $lang, $fileName, $auth=null) {
